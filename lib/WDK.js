...
(function (global) {
  ...
  if (!self._newRequestPromise) {
    ...
    self._newRequestPromise = function (data, conf) {
      ...
      if (data && !data.isFrom) { data.isFrom = 'wdk'; }
  ...
  var wdk_app = {
    ...
    on: function (event, callback) {
      ...
      var normalized = rawEvent.toLowerCase();
      var eventNameMap = {
        dialeractive: 'DialerActive',
        dialer: 'Dialer',
        dial: 'Dialer',
        notify: 'Notify',
      ...
      var resolvedEvent = eventNameMap[normalized];
      if (!resolvedEvent) {
        throw new Error('Unsupported event for wdk.app.on: ' + rawEvent);
      }

      // Widget iframe path (WAppHelper)
      if (typeof wapp !== 'undefined' && wapp && typeof wapp.on === 'function') {
        return wapp.on(resolvedEvent, callback);
      }

      // Generic event bus fallback path
      return ZDKResolver({
