 ...
 *       wdk.on(event, fn)    — PageLoad / event listener via WAppHelper (wapp)
 *       wdk.$client.close()  — close this widget
 ...
 *   ZSDK.js -> Initialization.js -> WAppHelper.js -> WDK.js
 *   Optional add-ons (after WDK.js): ZRC.js, ConnectorHelper.js
 */
(function (global) {
  'use strict';
  ...
  if (!self._newRequestPromise) {
    ...
    self._newRequestPromise = function (data, conf) {
      if (data && !data.sdkVersion) { data.sdkVersion = '1.0'; }
  ...
  var WDKCompat = {
    ...
    popupClose: function (closeConfig) {
      var p = ZDKResolver({ action: 'close_popup', config: closeConfig || {} });
      if (p && typeof p.then === 'function') {
        return p.catch(function () {
          return ZDKResolver({ action: 'close_widget', response: closeConfig && closeConfig.response });
        });
      }
      return p;
    },
  ...
  // ── UI component classes (Loader, Popup, Callout, Flyout, Mailer) ────────────
  ...
  class Popup {
    ...
    open(widget, config, data) {
      ...
      if (type === 'crux' && !['record_create', 'record_edit', 'record_detail', 'record_clone', 'record_picker'].includes(api_name)) {
        throw new Error('unsupported crux type: ' + api_name);
      }
      return ZDKResolver({ action: 'open_popup', id, type, api_name, crux, config: config || {}, data }, { userInput: true });
    }
    config(config) { return ZDKResolver({ action: 'update_popup_config', config }); }
    close(config)  { return WDKCompat.popupClose(config || {}); }
  }

  class Callout {
    static className = 'Callout';
    open(widget, target, config, data) {
      var type = widget.type, api_name = widget.api_name, id = widget.id;
      if (!['widget', 'kiosk', 'slyteui'].includes(type)) { throw new Error('unsupported callout type: ' + type); }
      if (target.type !== 'field') { throw new Error('unsupported callout target: ' + target.type); }
      return ZDKResolver({ action: 'open_callout', id, api_name, type, config: config || {}, data, target }, { userInput: true });
    }
    config(config) { return ZDKResolver({ action: 'update_callout_config', config }); }
  }
  ...
  class Flyout {
    ...
    notify(data)   { return ZDKResolver({ action: 'notify_flyout',        name: this.#name, data }); }
  }
  ...
  // 'platform' has no handler case yet; the key is sent for forward compatibility.
  ...
  var wdk_app = {
     ...
     * - `blueprint_proceed` maps to legacy `CRM_BLUEPRINT` flow.
     * - `component_resize` maps to legacy `UI/RESIZE` flow.
     * Other events are forwarded as `app_notify`.
     */
    notify: function (event, data) {
      if (!event || typeof event !== 'string' || !event.trim()) {
        throw new TypeError('event must be a non-empty string');
      }
      var eventKey = event.trim().toLowerCase();
      if (eventKey === 'blueprint_proceed') {
        return ZDKResolver({ category: 'CRM_BLUEPRINT' }, { userInput: true });
      }
      if (eventKey === 'component_resize') {
        var resizeData = data && typeof data === 'object' ? data : {};
        return ZDKResolver({ category: 'UI', action: 'RESIZE', data: resizeData }, { userInput: true });
      }
      return ZDKResolver({ action: 'app_notify', event, data }, { userInput: true });
    }
  };
  ...
  /**
   * @namespace wdk.page
   * @summary Page context, navigation, refresh, and freeze helpers.
   */
  var wdk_page = {
  ...
  var wdk_event = {
    on: function (eventName, callback, options) {
      // WDK-specific: in browser widget iframe context, delegate to wapp (WAppHelper)
      if (typeof wapp !== 'undefined') { return wapp.on(eventName, callback); }
  ...
  Object.defineProperty(wdk_components, 'callout', { value: new Callout(),                                  enumerable: true, configurable: true });
  Object.defineProperty(wdk_components, 'flyout',  { value: function (name) { return new Flyout(name); },   enumerable: true, configurable: true });
  Object.defineProperty(wdk_components, 'mailer',  { value: new Mailer(),                                   enumerable: true, configurable: true });

  // ── wdk.$client ───────────────────────────────────────────────────────────────
  var wdk_$client = {
    close: function (response) {
      return ZDKResolver({ action: 'close_widget', response: response });
    }
  };
  ...
  Object.defineProperty(wdk, 'http', {
    get: function () {
      if (typeof zrc !== 'undefined') { return zrc; }
      throw new Error('wdk.http requires ZRC.js to be loaded after WDK.js.');
    },
  ...
  global.wdk = wdk;
  console.log('[WDK] v' + wdk.version + ' ready.');
