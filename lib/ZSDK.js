var ZSDKUtil = (function(ZSDKUtil) {
  ...
  function GetQueryParams(URL) {
    ...
    if( qParams.hasOwnProperty('serviceOrigin') ) {
      ...
      qParams['serviceOrigin'] = decodeURIComponent(document.referrer);
    }
...
// Global setup: Store references for Bootstrap
var ZSDKGlobalQueryParams = null;
var ZSDKGlobalSDKContext = null;
var ZSDKBootstrapInitialized = false;

// Bootstrap function - runs once at startup to initialize message listener
function ZSDKBootstrap() {
  if (ZSDKBootstrapInitialized) {
    return;  // Only run once
  }
  
  ZSDKBootstrapInitialized = true;
  ZSDKGlobalQueryParams = ZSDKUtil.GetQueryParams();
  
  const parentOrigin = (function getParentOrigin(serviceOrigin) {
    const stored = sessionStorage.getItem('parentOrigin');
  ...
  })(ZSDKGlobalQueryParams.serviceOrigin);
  
  ZSDKGlobalQueryParams.serviceOrigin = parentOrigin;
  
  // Set up the message listener immediately (not deferred)
  var SDKContext = {
    isDevMode: !!ZSDKGlobalQueryParams.isDevMode,
    QueryParams: ZSDKGlobalQueryParams
  };
  ZSDKGlobalSDKContext = SDKContext;
...
// Call Bootstrap immediately when ZSDK.js loads
ZSDKBootstrap();
...
;function ZSDK(options) { // TODO: Replace console with Logger
  ...
  var connectors;
  var QueryParams = ZSDKGlobalQueryParams;  // Use global QueryParams
  var uniqueID;
  ...
  function getParentOrigin(serviceOrigin) {
    const stored = sessionStorage.getItem('parentOrigin');
    if (stored && stored != window.location.origin) return stored;
    
    // Fallback: try document.referrer again
    if(serviceOrigin){
      const origin = new URL(serviceOrigin).origin;
      sessionStorage.setItem('parentOrigin', origin);
      return origin;
  ...
  function InitializeInstance() {
    // Set up the SDKContext with this instance's functions
    ZSDKGlobalSDKContext.ExecuteLoadHandler = ExecuteLoadHandler;
    ZSDKGlobalSDKContext.SetContext = SetContext;
    ZSDKGlobalSDKContext.UpdateContext = UpdateContext;
    ZSDKGlobalSDKContext.GetConnectors = GetConnectors;
    ZSDKGlobalSDKContext.TriggerEvent = TriggerEvent;
    ZSDKGlobalSDKContext.ExecuteContextUpdateHandler = ExecuteContextUpdateHandler;
    ZSDKGlobalSDKContext.getUniqueID = getUniqueID;
    ZSDKGlobalSDKContext.isAppRegistered = isAppRegistered;

    if( typeof ZSDKModelManager !== 'undefined' ) {
      ZSDKModelManager.Init(ZSDKGlobalSDKContext);
    }

    RegisterClient();
  }

  InitializeInstance(); // Initialize this ZSDK instance
};
