...
;function ZSDK(options) { // TODO: Replace console with Logger
  ...
  function getParentOrigin(serviceOrigin) {
    ...
    if(serviceOrigin){
      ...
      sessionStorage.setItem('parentOrigin', origin);
      return origin;
    } else if(window.__REFERRER__) {
      const origin = new URL(window.__REFERRER__).origin;
      sessionStorage.setItem('parentOrigin', origin);
      return origin;
    } else if(window.location.protocol === "blob:") {
      const origin = new URL(window.location.pathname).origin;
      sessionStorage.setItem('parentOrigin', origin);
      return origin;
    } 
  }
  ...
  function Bootstrap() {
    ...
    const parentOrigin = getParentOrigin(QueryParams.serviceOrigin);
    QueryParams.serviceOrigin = parentOrigin;
    
    var SDKContext = {};
    SDKContext.isDevMode = !!QueryParams.isDevMode;
