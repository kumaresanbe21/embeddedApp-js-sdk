 ...
 * @property {function(): boolean} isAuthorized Returns whether the user-based connection is authorized.
 * @property {function(): void} authorize Triggers the authorization flow for the user-based connection.
 */
...
(() => {
	...
	/**
	 * Triggers the UI handler for authorizing a user-based connection.
	 * @param {{ api_name: string, authentication: { status: boolean, url: string } }} connection
	 * @returns {any}
	 */
	function ConnectionAuthUIHandler(connection) {
		const res = newRequestPromise({ action: 'handle_connection_auth', data: connection }); //no i18n
		if (!res) throw new ZrcError('Failed to get connection auth url');
		return res;
	}

	/**
	 * Returns whether the current context is a Client Portal environment.
	 * Result is cached after the first call.
	 * @returns {boolean}
	 */
	function isClientPortalEnv() {
		if (_isClientPortalCache === undefined) {
			_isClientPortalCache = !!newRequestPromise({ action: 'is_client_portal' }); //no i18n
		}
		return _isClientPortalCache;
	}

	/**
	 * Fetches and (optionally caches) the connection details for a given connection name.
	 * @param {string} connectionName
	 * @param {boolean} [fromCache=true]
	 * @returns {any} connection details object
	 */
	function getConnectionDetails(connectionName, fromCache = true) {
		if (isClientPortalEnv()) {
			throw new ZrcError('zrc.$.connection is not supported in Client Portal context'); //no i18n
		}
		...
		const res = newRequestPromise({
			action: 'api_dispatch', //no i18n
			data: {
				url: `/crm/v8/settings/connections/${connectionName}`, //no i18n
		...
		if (res && res.status_code) {
			...
			if (res.status_code === 204) {
				throw new ZrcError(`Connection not found: '${connectionName}'`); //no i18n
			}
		}
		throw new ZrcError(`Failed to fetch details for connection: '${connectionName}'`); //no i18n
	}
	...
	class ZrcConnection {
		...
		constructor(connectionName) {
			if (!connectionName || typeof connectionName !== 'string' || connectionName.trim() === '') {
				throw new ZrcValidationError('Please provide a valid connection name'); //no i18n
			}
		 ...
		 */
		isAuthorized() {
			const connDetails = getConnectionDetails(this.#connectionName, false);

			if (!connDetails.user_based) {
				throw new ZrcError(`Connection '${this.#connectionName}' does not support user-based authorization`); //no i18n
			}
		...
		authorize() {
			try {
				const connectionDetails = getConnectionDetails(this.#connectionName);
				...
				const res = ConnectionAuthUIHandler({ api_name, authentication });
				const defaultConnectionErrorMessage = `Failed to authorize the connection: ${this.#connectionName}`; //no i18n

				if (res.error) {
					throw new ZrcError(res.error.message || defaultConnectionErrorMessage);
				} else if (res.status && res.status !== 'success' && res.status !== true) { //no i18n
					throw new ZrcError(defaultConnectionErrorMessage);
				}
				return;
			} catch (error) {
				if (error instanceof ZrcError) throw error;
				throw new ZrcError(`Failed to authorize the connection: ${this.#connectionName}`); //no i18n
			}
		}
	}
...
const zrc = (() => {
	...
	return {
		 ...
		 * @type {{ connection: function(string): ZrcConnectionInstance }}
		 */
		$: {
			/**
			 * Get a Connection object to check authorization and authorize user-based connections.
			 * @param {string} connectionName - Name of the connection
			 * @returns {ZrcConnectionInstance}
			 * 
			 * @example
